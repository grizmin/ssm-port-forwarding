...
class TestCli(unittest.TestCase):
    ...
    def test_cli_version_ssmpfwd(self):
        ...
        self.assertRegex(result.output, r'^ssmpfwd/\d+.\d+.\d+',
                         'version should be semver')

    def test_cli_version_python(self):
        result = self.runner.invoke(cli.cli, ['--version'])
        assert result.exit_code == 0
        self.assertRegex(result.output, 'python/',
                         'python/ not found in version string')

    def test_cli_version_boto3(self):
        result = self.runner.invoke(cli.cli, ['--version'])
        assert result.exit_code == 0
        self.assertRegex(result.output, 'boto3/',
                         'boto3/ not found in version string')
    ...
    def test_cli_spawn_socat(self, ConsoleSMSession_mock):
        ...
        ConsoleSMSession_mock.assert_called_once_with('instance_id',
                    'profile_name', 'region_name', source_port=1234)
    ...
    def test_cli_forward(self,spawn_socat , spawn_port_forwarding_ssm_session):
        ...
        spawn_socat.assert_called_once_with('default', 'us-east-1', 'i-0616ea1bf6338cce6',
                                                source_port,
                                                '10.243.9.78:22')
        spawn_port_forwarding_ssm_session.assert_called_once_with('default', 'us-east-1',
                                                                       'i-0616ea1bf6338cce6',
                                                                       unittest.mock.ANY,
                                                                       unittest.mock.ANY)
        self.assertFalse(result.exit_code)
